 ...
 * @LastEditTime: 2024-07-13 22:13:07
...
void can_init(uint8_t TX_PIN = 8, uint8_t RX_PIN = 18,
    /*电流更新频率=*/int current_update_hz = 100);
...
void add_user_can_func(
    uint16_t addr, void (*func)(twai_message_t *can_message));
...
class C600_DATA {
	// 定义为友元，方便调用
	friend void feedback_update_task(
	    void *n); // 总线数据接收和发送线程，全局只有一个任务
	friend void update_current_task(void *n); // 更新电流任务，全局只有一个任务
	friend void location_contral_task(void *n);
	friend void speed_contral_task(void *
	        n); // 速度闭环控制任务线程，每个电机都有自己的速度闭环控制任务，并且与位置闭环控制任务不同时存在
	friend MOTOR;
	friend M3508_P19;
	friend M2006_P36;
	friend GM6020;
  ...
  public:
	...
	~C600_DATA() {}
	// 返回角度，范围：0-360，单位：度
	float get_angle() { return angle / 8192.0; }
	// 返回转子速度，单位：RPM
	int get_speed() { return speed; }
	// 返回电流
	int get_current() { return current; }
	// 获取电机温度，单位：摄氏度
	int get_tempertrue() { return tempertrue; }
	// 获取多圈位置,每8192为转子1圈
...
// 1-8号电机数据接收对象,用户无需访问
C600_DATA motor_201, motor_202, motor_203, motor_204, motor_205, motor_206,
    motor_207, motor_208 /*后面是GM6020专属地址→*/, motor_209, motor_20A,
    motor_20B;
C600_DATA *motors[] = {&motor_201, &motor_202, &motor_203, &motor_204,
    &motor_205, &motor_206, &motor_207, &motor_208, &motor_209, &motor_20A,
    &motor_20B};
...
class MOTOR {
  ...
  public:
	...
	void setup() {
		...
		if (speed_func_handle == nullptr) {
			xTaskCreate(speed_contral_task, "speed_contral_task", 4096, this, 2,
			    &speed_func_handle);
		}
	};
	// 判断电机是否在线
	bool is_online() { return data->is_online(); }
	...
	// 设置位置闭环控制参数
	void set_location_pid(float _location_Kp = 0, float _location_Ki = 0,
	    float _location_Kd = 0, float __dead_zone = 0, float _max_speed = 0) {
	...
	void set_location_pid(pid_param pid) {
		set_location_pid(
		    pid.Kp, pid.Ki, pid.Kd, pid._dead_zone, pid._max_value);
	}
	// 设置速度闭环控制参数
	void set_speed_pid(float _speed_Kp = 0, float _speed_Ki = 0,
	    float _speed_Kd = 0, float __dead_zone = 0, float _max_curunt = 0) {
	...
	void set_location(int64_t _location) {
		...
		if (location_func_handle == nullptr) {
			xTaskCreate(location_contral_task, "location_contral_task", 4096,
			    this, 2, &location_func_handle);
		}
	...
	// 获取当前多圈位置
	int64_t get_location() { return data->location; }
	int get_current_raw() { return data->get_current(); }
	...
	// 获取是否使能
	bool get_is_load() { return speed_func_handle != nullptr; }
	// 获取当前速度
	virtual float get_now_speed() { return data->speed; }
	...
	virtual void set_speed(float speed, float acce = 0) {
		...
		if (speed_func_handle == nullptr) {
			xTaskCreate(speed_contral_task,
			    "speed_cspeed_func_handleontral_task", 4096, this, 2,
			    &speed_func_handle);
		}
		taget_speed = speed;
		acceleration = acce;
	...
	// 获取减速箱减速比
	float get_reduction_ratio() { return reduction_ratio; }
   ...
   * @param (int (*func_ptr)(int64_t location))
   * 返回值为电流值，输入为位置的映射函数指针
   */
  ...
  protected:
	static int location_to_current(int64_t location) { return 0; }
	uint8_t ID;
	...
	pid_param default_location_pid_parmater = {
	    10, 0, 0, 2000, 3000};          // 位置闭环默认控制参数
	PID_CONTROL location_pid_contraler; // 位置闭环控制PID计算对象
	pid_param default_speed_pid_parmater = {
	    5, 1, 0.01, 1, 10000};       // 速度闭环默认控制参数
	...
	int speed_location_K =
	    1000; // 速度环位置误差系数,这里的比例系数需要根据实际情况调整,比例系数speed_location_K可以理解为转子每相差一圈加
	          // speed_location_K RPM速度补偿
};

// 3508类
class M3508_P19 : public MOTOR {
  public:
	M3508_P19(int id) : MOTOR(id) { reduction_ratio = 19.0; };
	M3508_P19(int id, pid_param location_pid, pid_param speed_pid)
	    : MOTOR(id, location_pid, speed_pid) {
	...
	float get_curunt_ma() { return 2e4 * data->current / 16384; }
	// 获取减速箱输出速度，单位RPM
	float get_now_speed() override { return data->speed / 19.0; }
};

// 2006电机类
class M2006_P36 : public MOTOR {
  public:
	M2006_P36(int id) : MOTOR(id) { reduction_ratio = 36.0; };
	M2006_P36(int id, pid_param location_pid, pid_param speed_pid)
	    : MOTOR(id, location_pid, speed_pid) {
	...
	float get_curunt_ma() { return 1e4 * data->current / 16384; }
	// 获取减速箱输出速度，单位RPM
	float get_now_speed() override { return data->speed / 36.0; }
};
  ...
  public:
	...
	void setup(int frc = 100) {
		if (GM6020_update_handle == nullptr) {
			int _frc = frc;
			xTaskCreate(GM6020_update, "GM6020_update", 4096, &_frc, 2,
			    &GM6020_update_handle);
		}
	}
	float get_curunt_ma() { return 3e3 * data->current / 16384; }

  protected:
	static void GM6020_update(void *m) {
		int frc = *(int *)m;
		while (1) {
			if ((motors[4]->enable && motors[4]->is_GM6020) ||
			    (motors[5]->enable && motors[5]->is_GM6020) ||
			    (motors[6]->enable && motors[6]->is_GM6020) ||
			    (motors[7]->enable && motors[7]->is_GM6020)) {
			...
			if ((motors[8]->enable && motors[8]->is_GM6020) ||
			    (motors[9]->enable && motors[9]->is_GM6020) ||
			    (motors[10]->enable && motors[10]->is_GM6020)) {
				twai_message_t tx_msg;
				tx_msg.data_length_code = 8;
...
void location_contral_task(void *n) {
	...
	while (1) {
		...
		speed = moto->location_pid_contraler.control(
		    moto->location_taget - moto->data->location);
...
void speed_contral_task(void *n) {
	...
	while (1) {
		...
		if (moto->acceleration == 0) {
			...
			// 如果启用了加速度控制
			if (abs(taget_control_speed - last_taget_control_speed) >
			    delta_time * moto->acceleration) {
				// 根据加速度控制速度
				taget_control_speed =
				    last_taget_control_speed + (taget_control_speed -
				                                   last_taget_control_speed) >
				            0
				        ? delta_time * moto->acceleration
				        : -delta_time * moto->acceleration;
			} else {
		...
		// 由速度计算得到的目标位置
		moto->speed_location_taget +=
		    8192.0 * taget_control_speed * delta_time / 60; // 位置误差
		...
		double err =
		    ...
		    /*速度环位置误差比例系数=*/moto
		            ->speed_location_K /*这里的比例系数需要根据实际情况调整,比例系数speed_location_K可以理解为转子每相差一圈加
                                      speed_location_K RPM速度补偿*/
		        *
		        /*由速度计算得到的目标位置的误差*/ (
		            moto->speed_location_taget - moto->data->location) /
		...
		// 计算控制电流
		int16_t cru =
		    /*位置映射的电流=*/moto->location_to_current_func_ptr(
		        moto->data->location) +
...
void update_current_task(void *p) {
	...
	while (1) {
		...
		if (motor_201.enable || motor_202.enable || motor_203.enable ||
		    motor_204.enable) {
			twai_message_t tx_msg;
			tx_msg.data_length_code = 8;
		...
		if (motor_205.enable || motor_206.enable || motor_207.enable ||
		    motor_208.enable) {
			twai_message_t tx_msg;
			tx_msg.data_length_code = 8;
...
void can_init(uint8_t TX_PIN, uint8_t RX_PIN, int current_update_hz) {
	...
	static const twai_general_config_t g_config = TWAI_GENERAL_CONFIG_DEFAULT(
	    gpio_num_t(TX_PIN), gpio_num_t(RX_PIN), TWAI_MODE_NO_ACK);
	...
	// 创建任务
	xTaskCreate(feedback_update_task, "moto_fb", 4096, nullptr, 5,
	    nullptr); // 电机反馈任务
	xTaskCreate(update_current_task, "update_current_task", 4096,
	    &current_update_hz, 5, nullptr); // 电流控制任务
	delay(100);
}
